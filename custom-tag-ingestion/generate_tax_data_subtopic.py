"""
Script to generate TAX_DATA dictionary from custom_tag_data.csv (without conceptId)
"""

import csv

def generate_tax_data(input_csv='custom_tag_data.csv', output_file='tax_data_subtopic.py'):
    """
    Read CSV and generate TAX_DATA dictionary
    
    CSV Format (tab-separated):
    taxonomyId	classId	subjectId	topicId	subtopicId	Qid	customTags
    """
    ...
    with open(input_csv, 'r', encoding='utf-8') as f:
        ...
        for line_num, line in enumerate(f, 1):
            try:
                ...
                if len(parts) != 7:
                    print(f"Warning: Line {line_num} has {len(parts)} columns (expected 7)")
                    continue
                
                taxonomy_id, class_id, subject_id, topic_id, subtopic_id, qid, custom_tags = parts
                
                # Create entry for this QId (without conceptId)
                tax_data[qid] = {
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('"""Auto-generated TAX_DATA from custom_tag_data.csv (Subtopic level)"""\n\n')
        f.write('TAX_DATA = {\n')
